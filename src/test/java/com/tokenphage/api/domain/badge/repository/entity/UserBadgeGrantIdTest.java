...
/**
 * 복합키(github_id + badge_code)의 equals()/hashCode()를 검증한다.
 * <p>
 * JPA가 같은 행인지 구분하는 기준이다. 어긋나면 중복 조회·중복 저장이 발생한다.
 */
class UserBadgeGrantIdTest {

    // 두 필드가 모두 같은 기준 인스턴스를 만들 때 쓰는 값
    private static final Long GITHUB_ID = -1L;
    private static final String BADGE_CODE = "contributor";

    private UserBadgeGrantId newId(Long githubId, String badgeCode) {
        return new UserBadgeGrantId(githubId, badgeCode);
    }

    /**
     * 기준값(두 필드 모두 동일)으로 채운 인스턴스를 만든다.
     */
    private UserBadgeGrantId baseId() {
    ...
    class EqualsTest {
        ...
        void equals_각필드별로하나만다름_false반환(String differingField) {
            ...
            UserBadgeGrantId b = switch (differingField) {
    ...
    class HashCodeTest {
        ...
        void hashCode_필드값다름_다른해시반환() {
            // given
            UserBadgeGrantId a = baseId();
            UserBadgeGrantId b = newId(-999L, "beta-tester");
